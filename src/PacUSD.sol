 ...
 */
contract PacUSD is ERC20Permit, Ownable, ReentrancyGuard {
    // Custom Errors
    error ZeroAmount();
    ...
    constructor(
        ...
        address _priceFeeds,
        bytes32 _pairId
    ) ERC20("PAC USD Stablecoin", "PacUSD") ERC20Permit("PAC USD Stablecoin") Ownable(msg.sender) ReentrancyGuard() {
        pacMMFWrapper = IERC20(_pacMMF);
        priceFeeds = ISupraPriceFeeds(_priceFeeds);
        pairId = _pairId;
        _totalSupply = 0;
        _totalShares = 0;  // 初始时总份额也应该是 0
        _sharesPerToken = SHARES_PER_TOKEN_PRECISION;  // 这个值会在第一次 mint 时被重新计算
        _lastRebasePrice = getPacMMFPrice();  // 使用实际的初始价格
    }
     ...
     * @param collateralAmount Amount of pacMMF to deposit
     */
    function mint(uint256 collateralAmount) external nonReentrant returns (uint256) {
     ...
     * @param pacUsdAmount Amount of PacUSD to burn
     */
    function burn(uint256 pacUsdAmount) external nonReentrant {
     ...
     * Can be called by anyone after REBASE_INTERVAL has passed
     */
